...
/**
 * Cluster integration tests for erasure coding over the gRPC transport.
 *
 * <p>Identical scenarios to {@link EcClusterIT} but the EC shard fan-out uses
 * {@link GrpcShardClient} instead of {@code HttpShardClient}.  Each node starts
 * a {@link GrpcServer} on a dedicated gRPC port; inter-node shard calls go
 * through the proto {@code ShardService} RPC instead of HTTP.
 *
 * <p>Verifies: gRPC EC write/read round-trip, degraded-mode shard reconstruction,
 * delete fan-out, HEAD Content-Length.
 */
class EcGrpcIT {
    ...
    private static final int PORT0  = 19310;
    private static final int PORT1  = 19311;
    private static final int PORT2  = 19312;
    private static final int RAFT0  = 16310;
    private static final int RAFT1  = 16311;
    private static final int RAFT2  = 16312;
    private static final int GRPC0  = 17310;
    private static final int GRPC1  = 17311;
    private static final int GRPC2  = 17312;

    private static final String BUCKET   = "ecg-test-bucket";
    ...
    static void startCluster() throws Exception {
        ...
        List<NodeId> peers    = List.of(
                new NodeId("ecg-node0", "localhost", PORT0, RAFT0, GRPC0),
                new NodeId("ecg-node1", "localhost", PORT1, RAFT1, GRPC1),
                new NodeId("ecg-node2", "localhost", PORT2, RAFT2, GRPC2));

        engine0 = new StorageEngine(Files.createTempDirectory("q1-ecg0-"), PARTITIONS);
        engine1 = new StorageEngine(Files.createTempDirectory("q1-ecg1-"), PARTITIONS);
        engine2 = new StorageEngine(Files.createTempDirectory("q1-ecg2-"), PARTITIONS);
        ...
        node0 = makeNode(peers, engine0, cluster0, PORT0, GRPC0);
        node1 = makeNode(peers, engine1, cluster1, PORT1, GRPC1);
        node2 = makeNode(peers, engine2, cluster2, PORT2, GRPC2);
        node0.start();
        node1.start();
        node2.start();
    ...
    void ecWriteAndReadFromAllNodes() throws Exception {
        String key  = "ecg-basic-" + System.nanoTime();
    ...
    void ecReconstructsWithOneMissingShardDeleted() throws Exception {
        String key  = "ecg-degrade-" + System.nanoTime();
        ...
        assertArrayEquals(data, get(PORT1, key), "Must reconstruct on node1");
    }

    @Test
    void ecDeleteRemovesAllShards() throws Exception {
        String key  = "ecg-delete-" + System.nanoTime();
    ...
    void headReturnsCorrectContentLength() throws Exception {
        String key  = "ecg-head-" + System.nanoTime();
    ...
    private static RatisCluster makeCluster(List<NodeId> peers, int selfIdx,
        ...
        ClusterConfig cfg = ClusterConfig.builder()
                ...
                .raftDataDir(Files.createTempDirectory("q1-ecgr-" + selfIdx + "-").toString())
                .ecConfig(ecConfig)
    ...
    private static Q1Server makeNode(List<NodeId> peers, StorageEngine engine,
                                     RatisCluster cluster, int httpPort, int grpcPort)
            throws Exception {
        ErasureCoder    coder  = new ErasureCoder(cluster.config().ecConfig());
        GrpcShardClient shards = new GrpcShardClient(peers);
        GrpcServer      grpc   = new GrpcServer(grpcPort, engine, cluster);
        return new Q1Server(engine, cluster, new PartitionRouter(cluster), coder, shards, httpPort)
                .withGrpcServer(grpc);
    }
