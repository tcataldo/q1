 ...
 */
public record EcConfig(int dataShards, int parityShards, int minObjectSize) {
    ...
    public EcConfig {
        ...
        if (minObjectSize < 0) throw new IllegalArgumentException("minObjectSize >= 0");
    }

    /** Convenience: k=0 → EC disabled, plain replication. */
    public static EcConfig disabled() {
        return new EcConfig(0, 0, 0);
    }
